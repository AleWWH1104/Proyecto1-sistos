...
std::vector<std::pair<std::string, chat::StatusEnum>>
UserRegistry::list_users() {
  std::lock_guard<std::mutex> lock(mtx_);
...
UserRegistry::get_user_info(const std::string &username) {
  std::lock_guard<std::mutex> lock(mtx_);
...
void UserRegistry::broadcast(const std::string &serialized_msg,
                             MessageType type,
