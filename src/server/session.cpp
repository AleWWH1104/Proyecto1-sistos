                    ...
                    UserRegistry &registry)
{
    ...
    {
        send_server_response(client_fd, 400,
                             "Debe registrarse primero", false);
        close(client_fd);
        return;
    }
    ...
    {
        send_server_response(client_fd, 409,
                             "Username or IP already registered", false);
        close(client_fd);
        return;
    }
    ...
    std::cout << "[Server] User '" << username
              << "' connected from " << client_ip << std::endl;
    ...
    while (running)
    {
        ...
        case MSG_CHANGE_STATUS:
        {
            ...
            send_server_response(client_fd, 200,
                                 "Status changed successfully", true);
            break;
        }
