...
struct UserInfo {
  ...
  bool inactive_auto; // true if INVISIBLE was set by inactivity (not user
                      // choice)
};
...
public:
  ...
  bool register_user(const std::string &username, const std::string &ip,
                     int socket_fd);

  // Remove user from registry. Returns true if found, false otherwise.
  bool unregister_user(const std::string &username);

  // Return list of {username, status} pairs, EXCLUDING users with status
  // INVISIBLE.
  ...
  // Find username by socket fd. Returns empty string if not found. O(1) via
  // reverse map.
  ...
  bool send_to_user(const std::string &username,
                    const std::string &serialized_msg, MessageType type);
...
private:
  ...
  std::unordered_map<int, std::string>
      fd_to_name_; // O(1) fd→username reverse map
